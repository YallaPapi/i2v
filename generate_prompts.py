"""
Dynamic i2i Prompt Generator

Generates bulk image-to-image prompts for use with the i2v bulk content generator.
Everything is generated dynamically by the LLM - no hardcoded locations or characters.

Output format: text file with prompts separated by --- (compatible with i2v bulk_generate.py)
"""
...
def get_random_caption_instructions(style="cosplay"):
...
def build_location_instructions(location_type, style="cosplay"):
    """Build location instructions based on type and style"""
    if style == "cottagecore":
        # Cottagecore uses rural locations
...
def build_style_instructions(style):
...
def generate_prompts(client, count, location_type, style="cosplay"):
    """Generate prompts using the LLM"""

    location_instructions = build_location_instructions(location_type, style)
    style_instructions = build_style_instructions(style)
    caption_instructions, category = get_random_caption_instructions(style)

    print(f"    Using caption category: {category}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate bulk i2i prompts")
    parser.add_argument("--count", type=int, default=10, help="Number of prompts to generate")
    parser.add_argument("--location", choices=["outdoor", "indoor", "mixed"], default="mixed",
                        help="Location type: outdoor, indoor, or mixed")
    parser.add_argument("--style", choices=["cosplay", "cottagecore"], default="cosplay",
                        help="Style: cosplay (anime characters, urban) or cottagecore (rural aesthetic)")
    ...
    print(f"  Location type: {args.location}")
    print("-" * 50)

    # Generate in batches if count is large (max 50 per batch)
    all_prompts = []
    ...
    while remaining > 0:
        ...
        print(f"Batch {batch_num}: generating {batch_size} prompts...")
    ...
    # Combine all batches and clean up
    all_text = "\n---\n".join(all_prompts)

    # Split by --- and clean each prompt (remove internal line breaks, make 1 per line)
    raw_prompts = all_text.split("---")
    clean_prompts = []
    ...
    # Join with single newline - one prompt per line, no separators
    final_output = "\n".join(clean_prompts)
    ...
    if args.output:
        ...
        output_file = prompts_dir / f"prompts_{timestamp}.txt"

    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(final_output)

    # Count actual prompts generated
    prompt_count = len(clean_prompts)

    print(f"\n{'='*50}")
    print(f"DONE! Generated {prompt_count} prompts")
    print(f"Saved to: {output_file}")
    print(f"\nTo use with i2v bulk_generate.py:")
    print(f"  python scripts/bulk_generate.py --prompts {output_file} --images <your_image_folder>")
